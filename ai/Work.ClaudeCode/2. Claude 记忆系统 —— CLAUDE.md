...
#### 公共 rules 和 个人 rules 怎么共存

可以共存，但要明确说明。公共规则要少而稳定；个人规则多而灵活可变。如果一个规则你离职后还可以用，这个规则就可以上升为一个公用规则。
...
> Claude code 团队对于 CC 关于Claude.md的一个最佳实践：
>
> 每次纠正完 Claude，都用这句话收尾：“更新你的 CLAUDE.md，这样你就不会再犯这个错误了。“Claude 在给自己写规则这件事上强得有点”诡异“。 随着时间推移，毫不留情地打磨你的 CLAUDE.md。持续迭代，直到你能明显量化地看到 Claude 的出错率下降。

> - CLAUDE.md  始终给我记着 
> - Rules 做某事前提醒一下 
> - Skill 具体做的时候，哪些特定操作，尤其是脚本，拿来用，别推理 
> - Command 好用的过程积累下来，下次懒得再重复相同过程

> CLAUDE.md是项目级，每次都加载。Skill Claude 判断需要时才加载，按需消耗，适合特定任务触发的多步骤工作流，比如说根据模板生成组件代码 。 
