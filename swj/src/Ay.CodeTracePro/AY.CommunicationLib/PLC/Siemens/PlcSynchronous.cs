...
using AY.CommunicationLib.PLC.Siemens.Helper;
...
namespace AY.CommunicationLib.PLC.Siemens
{
    public partial class Plc
    {
        ...
        public byte[] ReadBytes(StoreType dataType, int db, int startByteAdr, int count)
        {
        ...
        public object Read(StoreType dataType, int db, int startByteAdr, VarType varType, int varCount, byte bitAdr = 0)
        {
        ...
        public object ReadStruct(Type structType, int db, int startByteAdr = 0)
        {
            ...
            var resultBytes = ReadBytes(StoreType.DataBlock, db, startByteAdr, numBytes);
        ...
        public void WriteBytes(StoreType dataType, int db, int startByteAdr, byte[] value)
        {
            int localIndex = 0;
        ...
        /// <param name="value">Bytes to write. If more than 200, multiple requests will be made.</param>
        public void WriteBit(StoreType dataType, int db, int startByteAdr, int bitAdr, bool value)
        {
        ...
        public void WriteBit(StoreType dataType, int db, int startByteAdr, int bitAdr, int value)
        {
            if (value < 0 || value > 1)
        ...
        /// <param name="bitAdr">The address of the bit. (0-7)</param>
        public void Write(StoreType dataType, int db, int startByteAdr, object value, int bitAdr = -1)
        {
            if (bitAdr != -1)
            {
        ...
        private void ReadBytesWithSingleRequest(StoreType dataType, int db, int startByteAdr, byte[] buffer, int offset, int count)
        {
            try
            {
                //1.拼接报文
        ...
        private void WriteBytesWithASingleRequest(StoreType dataType, int db, int startByteAdr, byte[] value, int dataOffset, int count)
        {
            try
            {
        ...
        private byte[] BuildWriteBytesPackage(StoreType dataType, int db, int startByteAdr, byte[] value, int dataOffset, int count)
        {
            int varCount = count;
            // first create the header
        ...
        private byte[] BuildWriteBitPackage(StoreType dataType, int db, int startByteAdr, bool bitValue, int bitAdr)
        {
        ...
        private void WriteBitWithASingleRequest(StoreType dataType, int db, int startByteAdr, int bitAdr, bool bitValue)
        {
            try
            {
